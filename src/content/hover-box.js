import { calculateOptimalPosition, copyToClipboard as copyTextToClipboard, getLanguageName, showNotification } from '../shared/utils.js';
...
export class HoverBox {
  ...
  show(translationData) {
    ...
    this.currentTranslation = translationData || null;
    this.pronunciationRequestToken = null;
  ...
  positionBox() {
    ...
    const finalX = optimalPosition.x + window.scrollX;
    const finalY = optimalPosition.y + window.scrollY;
  ...
  async renderPronunciation(data) {
    if (!this.box || !this.pronunciationSectionEl || !this.pronunciationContentEl) return;

    this.pronunciationButtons.clear();
    this.pronunciationSectionEl.style.display = 'none';
    this.pronunciationContentEl.innerHTML = '';
    this.pronunciationRequestToken = null;
    ...
    this.pronunciationContentEl.innerHTML = PRONUNCIATION_LOADING_HTML;
    ...
    try {
      ...
      info.entries.forEach((entry) => {
        ...
        this.pronunciationContentEl.appendChild(item);
  ...
  async handleAccentPronounce(accent, word, button) {
    ...
    const fallback = () => {
      const locale = ACCENT_CONFIG[accent]?.locale || ACCENT_CONFIG.default.locale;
    ...
    if (targetEntry.audioUrl && typeof Audio === 'function') {
      try {
        ...
        audio.onerror = (event) => {
          console.warn('音频播放失败:', event?.message || event);
          finish();
          fallback();
        };
  ...
  speakWithSpeechSynthesis(locale, text, options = {}) {
    if (!text) {
      options.onError?.();
      return;
    }
  ...
  parsePronunciationResponse(data) {
  ...
  getAccentLabel(accent) {
    return ACCENT_CONFIG[accent]?.label || ACCENT_CONFIG.default.label;
  }

  getFallbackPronunciation(word) {
