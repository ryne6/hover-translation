import { GoogleTranslateAdapter } from '../adapters/traditional/GoogleTranslateAdapter.js';
import { BaiduTranslateAdapter } from '../adapters/traditional/BaiduTranslateAdapter.js';
import { DeepLAdapter } from '../adapters/traditional/DeepLAdapter.js';
import { MicrosoftTranslatorAdapter } from '../adapters/traditional/MicrosoftTranslatorAdapter.js';
import { YoudaoTranslateAdapter } from '../adapters/traditional/YoudaoTranslateAdapter.js';
import { TencentTranslateAdapter } from '../adapters/traditional/TencentTranslateAdapter.js';
...
export class AdapterFactory {
   ...
   */
  static registerAdapter(providerId, adapterClass) {
   ...
   * 创建适配器实例
   * @param {string} providerId - 提供商ID
   * @param {import('../interfaces/types.js').AdapterConfig} config - 配置
   * @returns {import('../interfaces/ITranslationAdapter.js').ITranslationAdapter}
   */
  static createAdapter(providerId, config) {
   ...
   * @returns {string|null}
   */
  static recommendProvider(sourceLang, targetLang) {
    // 语言对偏好映射
    const preferences = {
      // 中英互译
      'zh-CN-en': 'baidu',
    ...
    return preferences[key] || 'google'; // 默认使用 Google
  }
   ...
   * @returns {boolean}
   */
  static isLanguagePairSupported(providerId, sourceLang, targetLang) {
    const AdapterClass = this.adapters.get(providerId);
   ...
   * 获取提供商实例（如果存在）
   * @param {string} providerId - 提供商ID
   * @returns {import('../interfaces/ITranslationAdapter.js').ITranslationAdapter|null}
   */
