...
import { COMMON_LANGUAGES, getNativeLanguageName } from '../../utils/language-codes';
import type { TranslationRequest, TranslationResponse, ValidationResult, UsageInfo } from '../../interfaces/types';

type OpenAIHeaders = HeadersInit;

interface OpenAIChatChoice {
  message: { content: string };
}

interface OpenAIUsage extends UsageInfo {
  total_tokens: number;
}
...
export class OpenAIAdapter extends BaseTranslationAdapter {
  ...
  async translate(request: TranslationRequest): Promise<TranslationResponse> {
    ...
    const data = await this.retryRequest<OpenAIChatResponse>(async () =>
      this.makeRequest<OpenAIChatResponse>('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
  ...
  async detectLanguage(text: string) {
    const data = await this.makeRequest<OpenAIChatResponse>('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${this.config.apiKey}`,
        'Content-Type': 'application/json'
   ...
   * 构建系统提示词
   */
  buildSystemPrompt(request: TranslationRequest): string {
   ...
   * 计算成本
   */
  calculateCost(usage: UsageInfo & { total_tokens?: number }, model: string): number {
    ...
    return (usage.total_tokens || 0) * rate;
  }
}
