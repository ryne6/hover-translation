...
type ClaudeHeaders = HeadersInit;
...
export class ClaudeAdapter extends BaseTranslationAdapter {
  ...
  async translate(request: TranslationRequest): Promise<TranslationResponse> {
    ...
    const data = await this.retryRequest<ClaudeResponseData>(async () =>
      this.makeRequest<ClaudeResponseData>('https://api.anthropic.com/v1/messages', {
        method: 'POST',
        headers: {
  ...
  async detectLanguage(text: string) {
    const data = await this.makeRequest<ClaudeResponseData>('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: {
        'x-api-key': this.config.apiKey,
        'anthropic-version': '2023-06-01',
  ...
  calculateCost(usage: UsageInfo & { input_tokens: number; output_tokens: number }, model: string): number {
