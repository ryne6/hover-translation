...
type GeminiHeaders = HeadersInit;
...
export class GeminiAdapter extends BaseTranslationAdapter {
  ...
  async translate(request: TranslationRequest): Promise<TranslationResponse> {
    ...
    const data = await this.retryRequest<GeminiResponse>(async () =>
      this.makeRequest<GeminiResponse>(url, {
        method: 'POST',
        headers: {
  ...
  async detectLanguage(text: string) {
    ...
    const data = await this.makeRequest<GeminiResponse>(url, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      } as GeminiHeaders,
